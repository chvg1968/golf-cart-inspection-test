...
export async function sendToAirtable(
  ...
  if (!baseId || !tableName || !apiKey) {
    ...
    throw new Error(
      `Faltan variables de entorno de Airtable: ${missingVars.join(", ")}`,
    );
  }
  ...
  try {
    ...
    if (!searchResponse.ok) {
      ...
      throw new Error(
        `Error buscando en Airtable: ${searchData.error?.message || searchData.message || "Error desconocido"}`,
      );
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Error Airtable: ${responseData.error?.message || responseData.message || "Error desconocido"}`,
      );
    }
    ...
    if (error instanceof Error) {
      ...
      throw new Error(
        `Error procesando la solicitud a Airtable: ${error.message}`,
      );
    } else {
 ...
 */
export const updateAirtablePdfLink = async (
  recordId: string,
  pdfUrl: string,
): Promise<boolean> => {
  try {
    console.log(
      `Actualizando enlace PDF en Airtable para recordId ${recordId} con URL: ${pdfUrl}`,
    );
    ...
    if (!updateResponse.ok || updateData.error || updateData.message) {
      ...
      if (
        updateData.fields &&
        updateData.fields["Inspection Status"] !== "Signed"
      ) {
        console.error(
          "Status no se actualizó correctamente:",
          updateData.fields["Inspection Status"],
        );
        return false;
      } else if (updateData.fields) {
        // Verificar que fields exista
        console.log("¡Status actualizado correctamente a Signed!");
        return true;
      } else {
        console.warn(
          'La respuesta de Airtable no incluyó el campo "fields" actualizado.',
        );
