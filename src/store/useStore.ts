...
interface InspectionStateProperties {
  // Renombrado para evitar colisión de nombres
  ...
  initializeAuth: () => {
    ...
    if (token && userDataString) {
      try {
        ...
        console.error(
          "Zustand store: Error parsing user data from localStorage",
          error,
        );
  ...
  handleUndo: () => {
    ...
    if (currentStep > 0) {
      ...
      if (selectedProperty && !isGuestView) {
        saveDiagramMarks(selectedProperty.diagramType, previousPoints).catch(
          (error) => {
  ...
  handlePointsChange: (newPoints) => {
    ...
    const uniquePoints = newPoints.filter(
      (point, index, self) =>
        index ===
        self.findIndex(
          (p) => p.x === point.x && p.y === point.y && p.color === point.color,
        ),
    );
    ...
    if (selectedProperty && !isGuestView) {
      saveDiagramMarks(selectedProperty.diagramType, uniquePoints).catch(
        (error) => {
