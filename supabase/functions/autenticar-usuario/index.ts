...
serve(async (req) => {
  ...
  try {
    ...
    console.error(
      "Error parsing JSON body in 'autenticar-usuario':",
      jsonError.message,
    );
    return new Response(
      JSON.stringify({
  ...
  try {
    ...
    if (!userName || !password) {
      return new Response(
        JSON.stringify({ error: "Faltan userName o password" }),
        {
          status: 400, // Bad Request
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    if (userError || !userData) {
      console.error(
        "Error al buscar usuario o usuario no encontrado:",
    ...
    if (!jwtSecret) {
      console.error(
        "CUSTOM_JWT_SECRET no está configurado en las variables de entorno.",
      );
      return new Response(
        JSON.stringify({
          error: "Error de configuración del servidor para JWT.",
        }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    return new Response(
      JSON.stringify({
        token,
        user: { id: userData.id, userName: userData.user_name },
      }),
      {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      },
    ...
    console.error(
      "Error en la lógica principal de autenticar-usuario:",
      error.message,
      error.stack,
    );
    return new Response(
