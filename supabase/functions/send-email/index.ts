      ...
      return (
        ...
        (originUrl.port === allowedUrl.port ||
          (allowedUrl.hostname === "localhost" &&
            ["5173", "5174", "60943"].includes(originUrl.port)))
      );
    } catch {
    ...
    return new Response(null, {
      headers: {
        ...
        "Access-Control-Allow-Headers":
          "Content-Type, Authorization, X-Message-ID",
...
async function safeReadRequestBody(req: Request): Promise<string> {
  try {
    ...
    throw new Error(
      `No se pudo leer el cuerpo de la solicitud: ${error.message}`,
    );
  }
}
...
async function handleRequest(req: Request): Promise<Response> {
  ...
  try {
    ...
    console.log("Datos recibidos para envío de email:", {
      ...
      diagramPoints: data.diagramPoints
        ? data.diagramPoints.length
        : "Sin puntos",
    });
    ...
    if (data.diagramPoints && Array.isArray(data.diagramPoints)) {
      ...
      const invalidPoints = data.diagramPoints.filter(
        (point) =>
          !point ||
    ...
    if (data.type === "admin-alert" || data.adminAlert) {
      try {
        ...
        const adminEmailService = new EmailService(
        ...
        const adminContent =
          generarContenidoAlertaFormularioCreado(adminEmailData);

        // Enviar directamente usando el método unificado
        console.log(
          "Enviando alerta directa a administradores:",
          adminContent.to,
        );

        // El adminContent ya es compatible con la API de Resend
        const adminResult = await adminEmailService.sendAdminEmail(
          adminContent,
          "created",
        );
        ...
        return new Response(
          JSON.stringify({
            error: "Error enviando alerta admin",
